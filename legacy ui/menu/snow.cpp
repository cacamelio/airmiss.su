...
void Snowflake::Snowflake::Render(ImDrawList* draw_list)
{
    if (!draw_list)
        return;

    auto snow_color = c_color(255, 255, 255, 255 * g_menu.alpha);

    draw_list->AddCircleFilled(ImVec2(pos.x, pos.y), radius, snow_color.as_imcolor());
}
...
void Snowflake::Update(std::vector< Snowflake >& snow, vec3 mouse, vec3 windowPos, ImDrawList* draw_list)
{
    mouse.x -= windowPos.x;
    mouse.y -= windowPos.y;
